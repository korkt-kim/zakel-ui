import { compileSync } from "@zakel-ui/compiler";
import { Plugin } from "vite";
import path from "path";
import { buildSync } from "esbuild";
import _eval from "eval";
import { theme, sheet, generateHash, UserTheme } from "@zakel-ui/sheet";
...
export default function zakelUI(options?: PluginOptions): Plugin {
  ...
  dir.forEach((filePath) => {
    if (filePath.startsWith("zakel.config.")) configPath = filePath;
  });
  ...
  const virtualModuleId = "virtual:zakel-ui";
  ...
  return {
    ...
    config(config) {
      ...
      config.define = Object.assign(config.define, {
        "globalThis.__ZAKEL_USER_THEME__": JSON.stringify(userTheme),
      });
      return config;
    },
    transform(code: string, id: string) {
      if (id.includes("@zakel-ui")) return;
      if (!/\.(t|j)(s|sx)?$/.test(id)) return;
      if (/node_modules/.test(id)) return;
      if (!code.includes("@zakel-ui/core")) return;
...
` + result.code
    ...
    configResolved(config) {
      if (config.define && Object.keys(config.define).length > 0) {
        config.configFileDependencies.push(`${config.root}/zakel.config.ts`);
      }
      mode = config.command;
    },
  };
