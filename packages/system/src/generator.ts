import { Tokens, generateHash, theme } from "@zakel-ui/sheet";
...
export class StyleGenerator {
  ...
  constructor(props: StyledProps & PseudoProps, isDynamic = false) {
    ...
    /**
     * Find the theme style from the user theme defined in the `zakel.config.ts` file.
     * @example
    ...
    /**
     * If the argument value is a user theme defined in the `zakel.config.ts` file, it is converted,
  ...
  private static getClassNamePrefix(isDynamic = false) {
    if (process.env.NODE_ENV === "production") {
      const userPrefix = process.env.ZAKEL_CLASS_NAME_PREFIX;
      return userPrefix ? `${userPrefix}-` : "zakel-";
    }
