import { sheet } from "@zakel-ui/sheet";
  ...
  const visitor: PluginObj<PluginPass>["visitor"] = {
    Program: {
      enter(path) {
        ...
        // Create an import statement for the 'Box' component from '@zakel-ui/core'
        importBox(path, importedStyleFunctions);
        // Replace the 'k' function from '@zakel-ui/core' with the corresponding HTML tag
        ...
        // Traversal over the JSX elements in the Program node to identify Zakel-UI components,
        // processComponents(path, importedStyleFunctions);

        // Traversal over JSX opening elements, identifying Zakel-UI components and extracting their style props.
        // path.traverse({
