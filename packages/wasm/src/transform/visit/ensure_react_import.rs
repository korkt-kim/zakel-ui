...
impl<'a> VisitMut<'a> for EnsureReactImport<'a> {
     ...
     * Thus, this function adds a separate React import declaration at the beginning of the program with a unique alias "__ZAKEL_REACT__".
     * This unique alias allows Zakel's compiler to safely refer to React (e.g., when using React.forwardRef to wrap styled components) without interfering with user-defined variable names.
     */
    fn visit_program(&mut self, program: &mut Program<'a>) {
        ...
        let local = BindingIdentifier {
            span: SPAN,
            name: Atom::from("__ZAKEL_REACT__"),
...
mod test {
    ...
    fn test_ensure_react_import() {
        ...
        let source = Codegen::new()
            ...
            .build(program)
            .source_text;
        assert_eq!(source, "import __ZAKEL_REACT__ from \"react\";\n")
    }
}
