...
export const CSS_PATH = (() => {
  const cssPath = path.join(_dirname, "..", "assets", "zakel.css");
  if (!existsSync(cssPath)) writeFileSync(cssPath, "");
  return cssPath;
})();

type ZakelWebpackOptions = {
...
class ZakelUIWebpackPlugin {
  ...
  constructor(
    ...
    dir.forEach((filePath) => {
      if (filePath.startsWith("zakel.config.")) {
  ...
  apply(compiler: Compiler) {
    ...
    compiler.options.plugins.push(
      new compiler.webpack.DefinePlugin({
        ...(config
          ? {
              "globalThis.__ZAKEL_USER_THEME__":
    ...
    compiler.hooks.watchRun.tap("@zakel-ui/webpack-plugin:watchRun", () => {
      this.watchMode = true;
    });
