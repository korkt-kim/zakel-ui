...
impl<'a, 'b> VisitMut<'a> for ReplaceKWithBox<'a, 'b> {
    /**
     * Processes the JSXElement nodes in the AST and replaces the `k` syntax from `@zakel-ui/core`
                ...
                if let JSXElementName::MemberExpression(member_expr) = &elem.opening_element.name {
                    if let JSXMemberExpressionObject::Identifier(ident) = &member_expr.object {
                        if ident.name == k_alias {
                            ...
                            let is_zakel_default_attr =
                                JSXAttributeItem::Attribute(self.ast.alloc(JSXAttribute {
                                    span: SPAN,
                                    name: JSXAttributeName::Identifier(self.ast.alloc(
                                        JSXIdentifier {
                                            span: SPAN,
                                            name: Atom::from("IS_ZAKEL_DEFAULT"),
                                        },
                                    )),
                            ...
                            attrs.push(as_attr);
                            attrs.push(is_zakel_default_attr);
                        }
                    }
                }
...
mod test {
    ...
    fn test_ensure_react_import() {
        ...
        let source_text = "
            import {k, Box} from '@zakel-ui/core';
            export const App = () => {
        ...
        let source = Codegen::new()
            ...
            .build(program)
            .source_text;
        assert_eq!(source, "import { k, Box } from \"@zakel-ui/core\";\nexport const App = () => {\n\treturn <Box as=\"div\" IS_ZAKEL_DEFAULT={true}><Box as=\"span\" IS_ZAKEL_DEFAULT={true}>hello</Box></Box>;\n};\n")
    }
}
