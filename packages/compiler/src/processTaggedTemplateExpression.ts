...
function replaceTaggedTemplate(
  ...
  if (className) {
    ...
    // Assuming React is available in this file as __ZAKEL_REACT__, as it's being imported in `babel-plugin/src/ensureReactImport.ts`.
    const replacement = `__ZAKEL_REACT__.forwardRef((props, ref) => {
      const combinedClassName = [props.className, "${className}"].filter(Boolean).join(" ");
      return <${component} {...props} ref={ref} className={combinedClassName} IS_ZAKEL_DEFAULT />;
    })`;
