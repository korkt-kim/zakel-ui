import {
  componentList,
  defaultComponentTag,
} from "@zakel-ui/core/components/componentList";
import {
  JsxOpeningElement,
...
import { isStyledProp, isPseudoProps } from "@zakel-ui/system";

/**
 * Optimizes JSX elements by converting Zakel components to raw HTML elements
 ...
 *
 * @param {string} componentName - The name of the Zakel component.
...
function safeReplaceTagName(
  ...
  try {
    ...
    jsxElement.getAttribute("IS_ZAKEL_DEFAULT")?.remove();

    // if (process.env.NODE_ENV === "development") {
    //   console.log(
    //     `[Zakel UI] Optimize Success 🐻‍❄️: ${originalComponent} component has been optimized.`
    //   );
    // }
  } catch {
    // if (process.env.NODE_ENV === "development") {
    //   console.error(
    //     `[Zakel UI] Optimize Failed 🐻‍❄️: ${originalComponent} component has not been optimized`
    //   );
    // }
  }
}
